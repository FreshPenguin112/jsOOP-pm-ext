...
(async function (Scratch) {
  ...
  if (!vm.dogeiscutObject) {
    setTimeout(() => {
      if (!vm.dogeiscutObject) {
        vm.extensionManager
          .loadExtensionURL("https://extensions.penguinmod.com/extensions/DogeisCut/dogeiscutObject.js")
          .then(() => {
  ...
  function initBlockTools() {
    ...
    ScratchBlocks.FieldCustom.registerInput(
      ...
      (field) => {
        ...
        const dragCheck =
          parent.isInFlyout || srcBlock.svgGroup_.classList.contains("blocklyDragging") ? "none" : "all";
  ...
  class JSObject {
    ...
    toJSON() {
      try {
        ...
        try {
          ...
          try { if (DEBUG) console.dir(s); return JSON.parse(s); } catch (_) { return s; }
      ...
      try {
        ...
        if (t === "object") {
          try {
            ...
            return v && v.constructor && v.constructor.name ? `[object ${v.constructor.name}]` : "[object]";
          }
        }
    ...
    toMonitorContent() { return this.toReporterContent(); }
    toListItem() { return this.toReporterContent(); }
    toListEditor() { return this.toString(); }
    fromListEditor(edit) { try { this.value = JSON.parse(edit); } catch { this.value = edit; } return this; }
    static toType(x) {
      ...
      if (x && typeof x === "object" && x.customId && typeof x.customId === "string") {
        try { if (vm && vm.runtime && vm.runtime.serializers && vm.runtime.serializers[x.customId]) return new JSObject(x); } catch (_) {}
    ...
    static prepareForSerialize(v) {
      ...
      if (v && typeof v === "object" && v.customId && vm && vm.runtime && vm.runtime.serializers && vm.runtime.serializers[v.customId]) {
        try { return { _nestedCustom: true, typeId: v.customId, data: vm.runtime.serializers[v.customId].serialize(v) }; } catch (e) {}
      }
      if (typeof v === "function") return { _functionSource: v.toString() };
      try { const json = safeSerialize(v); return { _json: json }; } catch (e) { return { _string: String(v) }; }
    }
      ...
      try {
        if (obj && typeof obj === "object") {
          if (obj._jsoopLookupMarker && obj.lookupId) {
            ...
            return new JSObject({ _jsoopLookupMissing: true, originalLookupId: obj.lookupId });
          }
          if (obj._nestedCustom && obj.typeId && vm.runtime.serializers[obj.typeId]) return vm.runtime.serializers[obj.typeId].deserialize(obj.data);
          if (obj._functionSource && typeof obj._functionSource === "string") {
            try { const fn = eval("(" + obj._functionSource + ")"); return fn; } catch (e) { try { return eval(obj._functionSource); } catch (ee) { return obj._functionSource; } }
          }
          if (obj._json) { try { return JSON.parse(obj._json); } catch (e) { return obj._json; } }
  ...
  class JSOOPExtension {
    ...
    _storeInLookupTable(jsObject) {
        ...
        return {
          ...
          toString: () =>
            new JSObject().toString.apply({
              value: this._convertToNativeValue(this._getFromLookupTable(builtInLookupId)),
            }),
          toJSON: () => ({
            _jsoopLookupMarker: true,
            lookupId: builtInLookupId,
      ...
      return {
        ...
        toString: () =>
          new JSObject().toString.apply({
            value: this._convertToNativeValue(this._getFromLookupTable(lookupId)),
          }),
        toJSON: () => ({
          _jsoopLookupMarker: true,
          lookupId: lookupId,
    ...
    _makeClassFromMethods(methods, envNames = [], envValues = []) {
      ...
      try {
          ...
          try {
            if (
              ...
              try {
                ...
                try {
                  const f2 = vm.exports.jsexecute.scopedEval("(" + code + ")");
        ...
        const convertProceduresInEnv = (names, values) => {
          if (!Array.isArray(names) || !Array.isArray(values)) return values;
          const idx = names.indexOf("procedures");
          if (idx >= 0 && values[idx] && typeof values[idx] === "object") {
            const srcObj = values[idx];
            const out = {};
            for (const k of Object.keys(srcObj)) {
              const v = srcObj[k];
              out[k] = typeof v === "function" ? v : stringToFunction(v) || v;
            }
        ...
        for (const m of methods || []) {
          ...
          try {
            if (
              (factorySrc.startsWith('"') && factorySrc.endsWith('"')) ||
              (factorySrc.startsWith("'") && factorySrc.endsWith("'"))
            ) {
          ...
          try {
            ...
            const methodEnvNames = Array.isArray(envNames) ? envNames.slice() : [];
            const methodEnvValues = Array.isArray(envValues) ? envValues.slice() : [];
            ...
            let procVal = m && m.procedures != null ? m.procedures : null;
            ...
            if (procVal && typeof procVal === "object") {
              ...
              for (const k of Object.keys(procVal)) {
                const v = procVal[k];
                methodProcedures[k] = typeof v === "function" ? v : stringToFunction(v) || v;
              }
              const procIdx = methodEnvNames.indexOf("procedures");
              if (procIdx === -1) {
            ...
            const hasEnv = Array.isArray(methodEnvNames) && methodEnvNames.length > 0;
            if (hasEnv) {
              // If env contains procedures, convert any stringified functions there first.
              const preparedEnvValues = convertProceduresInEnv(methodEnvNames, methodEnvValues);

              const envList = methodEnvNames.map((n) => String(n));
              ...
              if (
                ...
                try {
                  const wrapperFn = vm.exports.jsexecute.scopedEval(wrapperSrc);
                  factoryFn = wrapperFn;
                } catch (e) {
                  try {
                    const wrapperFn = vm.exports.jsexecute.scopedEval("(" + wrapperSrc + ")");
              ...
              if (!factoryFn) {
                try {
                  const wrapperFn = new Function("return " + wrapperSrc)();
                  factoryFn =
                    typeof wrapperFn === "function" ? wrapperFn.apply(null, preparedEnvValues) : wrapperFn;
                } catch (e) {
                  try {
                    factoryFn = new Function("return (" + wrapperSrc + ")")();
                    if (typeof factoryFn === "function") factoryFn = factoryFn.apply(null, preparedEnvValues);
                  } catch (e2) {
            ...
            if (!factoryFn) {
              try {
                if (
                  ...
                  try {
                    const maybe = vm.exports.jsexecute.scopedEval(factorySrc);
                    if (typeof maybe === "function") {
                      factoryFn = maybe;
                    }
                  } catch (err) {
                    try {
                      const maybe2 = vm.exports.jsexecute.scopedEval("(" + factorySrc + ")");
                      if (typeof maybe2 === "function") factoryFn = maybe2;
                    } catch (_) {}
                  }
                }
              } catch (_) {}
              ...
              if (!factoryFn) {
                try {
                  factoryFn = new Function("return " + factorySrc)();
                } catch (e) {
                  try {
                    factoryFn = new Function("return (" + factorySrc + ")")();
                  } catch (e2) {
            ...
            try {
              ...
              console.error("_makeClassFromMethods: failed to evaluate method factory for", name, e2);
              factoryFn = null;
            }
          }
          ...
          if (!factoryFn || typeof factoryFn !== "function") {
            ...
            try {
              ...
              console.error("_makeClassFromMethods: wrapper eval also failed for", name, e2);
              try {
                const snippet = factorySrc ? String(factorySrc).slice(0, 200) : "<empty>";
                console.error("_makeClassFromMethods: failed factorySrc snippet:", snippet);
              } catch (_) {}
              factoryFn = null;
            }
          }
          ...
          if (typeof factoryFn === "function") {
            try {
              ...
              if (typeof maybe === "function" || (maybe && typeof maybe.next === "function")) {
                fn = maybe;
              } else {
                fn = factoryFn;
              }
          ...
          try {
            ...
            try {
              if (m && m.firstBlockId) {
                Object.defineProperty(fn, "_jsoopFirstBlockId", {
            ...
            try {
              if (m && m.thread) {
                Object.defineProperty(fn, "_jsoopAttachedThread", {
                  value: m.thread,
                  writable: false,
                  configurable: true,
                });
            ...
            try {
              ...
              Object.defineProperty(fn, "_procedures", {
                value: typeof methodProcedures === "object" && methodProcedures ? methodProcedures : {},
                writable: false,
                configurable: true,
              });
            } catch (_) {}
          ...
          if (cleanName === "constructor" && !type.includes("static")) {
        ...
        try {
          ...
          for (const key of protoKeys) {
            ...
            try {
              ...
              let src = desc.value;
              try { if ((src.startsWith('"') && src.endsWith('"')) || (src.startsWith("'") && src.endsWith("'"))) src = JSON.parse(src); } catch (_) {}
              let conv = null;
              try { conv = stringToFunction(src); } catch (_) { conv = null; }
              if (typeof conv !== 'function') {
                try { conv = new Function('return ' + src)(); } catch (_) { conv = null; }
              }
              if (typeof conv === 'function') {
                Object.defineProperty(C.prototype, key, { value: conv, writable: true, configurable: true });
              }
            } catch (_) {}
          }
        } catch (_) {}
          ...
          try {
            ...
            const Wrapped = class extends Base {
              constructor(...ctorArgs) {
                ...
                try {
                  const callerThread =
                    this && this.constructor && this.constructor._jsoopCallerThread
                      ? this.constructor._jsoopCallerThread
                      : typeof globalState !== "undefined" && globalState && globalState.thread
                        ? globalState.thread
                  ...
                  console.error("constructor wrapper failed", e);
                }
              }
            };
            ...
            try {
              const protoKeys2 = Object.getOwnPropertyNames(Wrapped.prototype);
              for (const key of protoKeys2) {
                if (key === 'constructor') continue;
                try {
                  const desc = Object.getOwnPropertyDescriptor(Wrapped.prototype, key);
                  if (!desc || typeof desc.value !== 'string') continue;
                  let src = desc.value;
                  try { if ((src.startsWith('"') && src.endsWith('"')) || (src.startsWith("'") && src.endsWith("'"))) src = JSON.parse(src); } catch (_) {}
                  let conv = null;
                  ...
                  if (typeof conv === 'function') {
                    Object.defineProperty(Wrapped.prototype, key, { value: conv, writable: true, configurable: true });
                  }
                } catch (_) {}
              }
            } catch (_) {}
            ...
            console.error("_makeClassFromMethods: failed to create constructor wrapper", e);
            return C;
          }
        }
    ...
    _getJsoopParamNames(fn) {
      ...
      return fn._jsoopParams.map((param) => {
        try {
          let p = param;
          if (p && typeof p === 'object' && p._jsoopLookupMarker && p.lookupId) {
            try { p = this._getFromLookupTable(p.lookupId); } catch (_) { p = undefined; }
          ...
          if (p && typeof p === 'object') {
            ...
            const match = String(key).trim().match(/^\.{0,3}\s*([A-Za-z_$][\w$]*)/);
    ...
    async _invokeJsoopFactory(fn, thisArg, callerThread, args) {
      ...
      for (let i = 0; i < paramNames.length; i++) {
        // Determine the raw param descriptor (may be a string, an object key, a JSObject, or a lookup marker)
        let rawName = null;
        try {
          const p = fn && fn._jsoopParams ? fn._jsoopParams[i] : undefined;
          let obj = p;
          if (obj && typeof obj === 'object' && obj._jsoopLookupMarker && obj.lookupId) {
            try { obj = this._getFromLookupTable(obj.lookupId); } catch (_) { obj = undefined; }
        ...
        // Normalize quoted legacy names like '"rest"' -> rest
        ...
        if (isSpread) {
          ...
          const value = i < args.length ? this._getActualValue(args[i]) : undefined;
          const key = maybeName;
      ...
      // 🔑 Critical: Provide a dummy stack entry so sequencer doesn't discard the thread.
      // The dummy block ID can be anything non‑null; it won't be executed.
      execThread.pushStack("__jsoop_dummy__");

      // Helper to convert stringified function source into a real function.
      // Prefer VM scopedEval when available, otherwise fall back to new Function.
        ...
        if (looksLikePlainObject && !isWrapperLike) {
          ...
          for (const k of Object.keys(_argsCandidate)) {
            ...
            try {
              ...
              // Try unwrapping quoted subname for spread-like keys '..."rest"'
              if (nk && nk.startsWith('...')) {
                const sub = nk.slice(3);
                try {
                  const parsed2 = JSON.parse(sub);
                  if (typeof parsed2 === 'string') nk = '...' + parsed2;
                } catch (_) {}
              }
            }
            try {
              ...
              if ((t.startsWith('"') && t.endsWith('"')) || (t.startsWith("'") && t.endsWith("'"))) {
                try { nk = JSON.parse(t); } catch (_) { nk = t; }
              } else {
                nk = t;
              }
      ...
      // functions), convert its entries to real functions and attach to the
      // callerThread as `procedures` so compiled factories can access them.
      ...
      try {
        ...
        if (fn && fn._procedures) {
          proceduresSrc = objectMap(fn._procedures, (p) => p(callerThread));
          //console.log(proceduresSrc);
        ...
        if (proceduresSrc && typeof proceduresSrc === "object") {
          ...
          for (const k of Object.keys(proceduresSrc)) {
            ...
            // Try to parse JSON-wrapped strings, then evaluate into a real function
            let fnConverted = v;
            try {
              if (
                typeof v === "string" &&
        ...
        // don't let conversion errors block factory invocation
        try {
          callerThread.procedures = _prevProcedures;
        } catch (_) {}
      }

      // Invoke factory – it returns a generator function or iterator
      const factoryResult = fn.call(thisArg, callerThread);
      let generator;
      if (typeof factoryResult === "function" && this._isGeneratorFunction(factoryResult)) {
        generator = factoryResult();
      } else if (factoryResult && typeof factoryResult.next === "function") {
        generator = factoryResult;
      } else {
        // Not a generator – restore any previous args and return value directly
        if (typeof _prevJsoopArgs === "undefined") {
          delete callerThread.jsoopArgs;
        } else {
          callerThread.jsoopArgs = _prevJsoopArgs;
        }
      ...
      // Optional: For safety, wrap the generator's step to catch errors
      ...
      //console.log(execThread);
      runtime.emit("THREAD_STARTED", execThread);

      // Wait for thread completion. Restore caller's jsoopArgs and prefer
      // execThread.justReturned but fall back to callerThread.justReturned
      // (some compiled code may set the return value on the original thread).
      return new Promise((resolve, reject) => {
        const finishHandler = (finishedThread) => {
          if (finishedThread === execThread) {
            runtime.removeListener("THREAD_FINISHED", finishHandler);
            // Prefer the execThread's justReturned, fall back to callerThread
            let ret = typeof finishedThread !== "undefined" ? finishedThread.justReturned : undefined;
            if (
            ...
            // Restore previous jsoopArgs on the caller thread
            try {
              if (typeof _prevJsoopArgs === "undefined") {
                delete callerThread.jsoopArgs;
              } else {
            ...
            // Restore previous procedures on the caller thread
            try {
        ...
        // Handle case where thread finishes synchronously before listener attached
          ...
          classMethod: (node, compiler, imports) => {
            ...
            if (firstBlockId) {
              ...
              try {
                ...
                const child = Object.create(Object.getPrototypeOf(compiler));

                Object.assign(child, compiler);
                child.source = "";
                child.inClassMethod = true;
                if (child.localVariables && typeof child.localVariables.clone === "function") {
                  child.localVariables = child.localVariables.clone();
                }
                if (firstBlockId) {
                  const test = new vm.exports.Thread(firstBlockId);
                  test.target = vm.runtime.targets.find((t) => firstBlockId in t.blocks._blocks);
                  if (test.target) {
                    ...
                    const compileScript = (script) => {
                      ...
                      const compiler = new vm.exports.JSGenerator(script, ir, test.target);
                    ...
                    for (const procedureVariant of Object.keys(ir.procedures)) {
                      ...
                      procedures[procedureVariant] = procedureTree.toString();
                    }
                    let js = null;
                    try {
                    ...
                    if (typeof js === "string" && js.trim()) {
                      ...
                      try {
                        if (js && typeof js.toString === "function") maybeString = js.toString();
                      } catch (_) {
                        maybeString = null;
                      }
                      if (typeof maybeString === "string" && maybeString.trim() && maybeString !== "[object Object]") {
                        factorySource = maybeString;
                      } else {
                        factorySource = child && child.source ? child.source : "(function empty(thread) {return;})";
                      }
                    }
                  } else {
        ...
        return { error: String(e) };
          ...
          classThis: (node, compiler, imports) => {
            if (compiler.inClassMethod) {
              return new imports.TypedInput("thread._jsoopThis", imports.TYPE_UNKNOWN);
            } else {
          ...
          classArgStack: (node, compiler, imports) => {
            const rawName = node.NAME ? compiler.descendInput(node.NAME).asString() : "";
            let nameVal = rawName;
            try { nameVal = JSON.parse(rawName); } catch (_) {}
          ...
          classArgStringStack: (node, compiler, imports) => {
            ...
            let nameVal = rawName;
            try { nameVal = JSON.parse(rawName); } catch (_) {}
            const defaultValue = node.DEFAULT ? compiler.descendInput(node.DEFAULT).asUnknown() : "undefined";
            const key = JSON.stringify(nameVal);
          ...
          constantUndefined: (node, compiler, imports) =>
            new imports.TypedInput("undefined", imports.TYPE_UNKNOWN),
          ...
          constantFunction: (node, compiler, imports) =>
            new imports.TypedInput("Function", imports.TYPE_UNKNOWN),
          ...
          constantGlobalThis: (node, compiler, imports) =>
            new imports.TypedInput("globalThis", imports.TYPE_UNKNOWN),
          constantInfinity: (node, compiler, imports) =>
            new imports.TypedInput("Infinity", imports.TYPE_UNKNOWN),
       ...
       yield* waitPromise(result);
          ...
          setSetting: (node, compiler, imports) => {
            ...
            const map = {
              ...
              "Automatic spread arg in class args object to jwArray conversion": "automaticSpreadArgInClassArgsObjectToJwArrayConversion",
...
`;
    ...
    _convertJwArrayToArgs(jwArrayObj) {
      if (jwArrayObj instanceof jwArray.Type) {
        ...
        if (this.settings && this.settings.automaticArgArrayToNativeConversion === false) {
    ...
    constantAsyncFunction() {
      return this._wrapForOtherExtensions(
        new JSObject(Object.getPrototypeOf(async function () {}).constructor),
      );
    }
    ...
    callMethod({ METHOD, INSTANCE, ARGS }, util) {
        ...
        if (typeof fnPrim === "function") {
          try {
            ...
            const result = primIsFactory
              ? this._invokeJsoopFactory(fnPrim, target, util && util.thread, args)
        ...
        if (typeof fnProto === "function") {
          try {
            ...
            const result = protoIsFactory
              ? this._invokeJsoopFactory(fnProto, target, util && util.thread, args)
      ...
      try {
        ...
        const callArgs = isFactory ? args : fn && fn._jsoopMethod ? [util && util.thread].concat(args) : args;
        const result = isFactory
          ? this._invokeJsoopFactory(fn, target, util && util.thread, args)
        ...
        if (typeof fnPrim === "function") {
          try {
            ...
            const res = primIsFactory
              ? await this._invokeJsoopFactory(fnPrim, target, util && util.thread, args)
              : fnPrim.apply(target, primCallArgs);
      ...
      try {
        ...
        const callArgs = isFactory
          ...
          : func && func._jsoopMethod
            ...
            : args;
        const result = isFactory
          ? this._invokeJsoopFactory(func, thisArg, util && util.thread, args)
    ...
    async awaitCallFunction({ FUNC, THIS, ARGS }, util) {
      ...
      try {
        ...
        let result = isFactory
          ? await this._invokeJsoopFactory(func, thisArg, util && util.thread, args)
          : func.apply(thisArg, func && func._jsoopMethod ? [util && util.thread].concat(args) : args);
        if (result && typeof result.then === "function") {
          result = await result;
        }
        ...
        try {
          ...
          const callArgs = isFactory
            ...
            : fn && fn._jsoopMethod
              ...
              : args;
          let result = isFactory
            ? await this._invokeJsoopFactory(fn, target, util && util.thread, args)
            : fn.apply(target, callArgs);
          if (result && typeof result.then === "function") {
            await result;
          }
    ...
    setSetting({ SETTING, VALUE }) {
      ...
      try {
        ...
        if (DEBUG)
          console.dir({ action: "setSetting(done)", key, val, settings: this.settings });
      } catch (err) {
    ...
    setPropJSObject({ PROP, INSTANCE, VALUE }) {
      ...
      const value =
        ...
        (VALUE && typeof VALUE === "object" && (VALUE._jsoopLookupMarker || VALUE.customId))
          ? VALUE
    ...
    classArgStack({ NAME }, util) {
      try {
        ...
        const topArgs = thread._jsoopArgsStack[thread._jsoopArgsStack.length - 1];
        if (topArgs) topArgs.push(new this.JSObject({ [String(NAME)]: undefined }));
      } catch (_) {}
    }
    classArgStringStack({ NAME, DEFAULT }, util) {
      try {
        ...
        thread._jsoopArgsStack ??= [];
        const topArgs = thread._jsoopArgsStack[thread._jsoopArgsStack.length - 1];
        if (topArgs) topArgs.push(this._storeClassArgDefault({ [String(NAME)]: DEFAULT }));
      } catch (_) {}
    }
