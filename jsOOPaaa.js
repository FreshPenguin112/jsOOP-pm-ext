      ...
      try {
        ...
        return { error: String(e) };
          ...
          argsBuilder: (node, compiler, imports) => {
            ...
            compiler.source += `try { let spreadCount = 0; for (let i = 0; i < ${arrVar}.length; i++) { let item = ${arrVar}[i] instanceof vm.runtime.ext_jsoop.JSObject ? ${arrVar}[i].value : ${arrVar}[i]; if (item && typeof item === 'object') { const keys = Object.keys(item); const key = keys[0]; if (String(key).startsWith('...')) { spreadCount++; if (spreadCount > 1) throw new Error('Only one spread arg is allowed'); if (i !== ${arrVar}.length - 1) throw new Error('Spread arg must be last'); } } } } catch (e) {}\n`;
